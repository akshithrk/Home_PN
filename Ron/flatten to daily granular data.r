make_patient_daily_dataframe<- function(targetmrn) {
        ...
        else lastdate <- data_export_date
        ...
        patient_daily.df <- data.frame(mrn=targetmrn,caldate=as.Date(caldate,"1970-01-01"),isactive=0,newhpn=0,admit=0,centline=0,nothosp=1,bloodinf=0)
...
make_all_patients_daily_dataframe <- function() {
        ...
        if(!exists("data_export_date"))
                stop(Print("data_export_date does not exist."))
        else if (is.na(data_export_date))
                stop("data_export_date is NA")

        all_patients_daily.dat <- data.frame(mrn=numeric(),caldate=as.Date(integer(),"1970-01-01"),isactive=integer(),newhpn=integer(),
        ...
        for (mrn in demog.dat$mrn) {
                ...
                if (is.null(x)) next
                else all_patients_daily.dat <- rbind(all_patients_daily.dat,x)
        }

        all_patients_daily.dat <<- all_patients_daily.dat
}

# write.csv(all_patients_daily.dat,"cldetail.csv")